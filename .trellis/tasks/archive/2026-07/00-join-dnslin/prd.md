...
`dnslin` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/dnslin/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/dnslin/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python ./.trellis/scripts/task.py list --assignee dnslin` to
...
python ./.trellis/scripts/task.py archive 00-join-dnslin
```
