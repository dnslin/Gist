...
describe("url utils", () => {
  ...
  describe("getSafeHostname", () => {
    ...
    it("should handle subdomains", () => {
      expect(getSafeHostname("https://blog.example.com/post")).toBe(
        "blog.example.com",
      );
    });

    it("should handle urls with ports", () => {
      expect(getSafeHostname("https://example.com:8080/path")).toBe(
        "example.com",
      );
    });
  ...
  describe("normalizeUrl", () => {
    ...
    it("should convert feed:// to https://", () => {
      expect(normalizeUrl("feed://example.com/rss")).toBe(
    ...
    it("should handle feed:// with path and query", () => {
      expect(normalizeUrl("feed://example.com/rss.xml?format=atom")).toBe(
        "https://example.com/rss.xml?format=atom",
      );
    });

    it("should handle domains with subdomains", () => {
      expect(normalizeUrl("blog.example.com/feed")).toBe(
        "https://blog.example.com/feed",
      );
    });

    it("should handle domains with ports", () => {
      expect(normalizeUrl("example.com:8080/rss")).toBe(
        "https://example.com:8080/rss",
      );
    });
