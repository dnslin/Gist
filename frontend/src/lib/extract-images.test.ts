...
describe("extract-images", () => {
  describe("extractImagesFromHtml", () => {
    it("should extract image URLs from img tags", () => {
      const html =
        '<img src="https://example.com/image1.jpg"><img src="https://example.com/image2.jpg">';
    ...
    it("should filter out data URIs", () => {
      const html =
        '<img src="data:image/png;base64,abc123"><img src="https://example.com/real.jpg">';
      const result = extractImagesFromHtml(html);
      expect(result).toHaveLength(1);
    ...
    it("should prefer src over data-src when both exist", () => {
      const html =
        '<img src="https://example.com/real.jpg" data-src="https://example.com/lazy.jpg">';
      const result = extractImagesFromHtml(html);
      expect(result).toHaveLength(1);
      expect(result[0]).toBe("https://example.com/real.jpg");
    });
    ...
    it("should handle images with query parameters", () => {
      const html =
        '<img src="https://example.com/image.jpg?width=800&quality=90">';
      const result = extractImagesFromHtml(html);
      expect(result).toHaveLength(1);
      expect(result[0]).toBe(
        "https://example.com/image.jpg?width=800&quality=90",
      );
    });

    it("should handle images with special characters in URL", () => {
      const html =
        '<img src="https://example.com/path/to/image%20with%20spaces.jpg">';
      const result = extractImagesFromHtml(html);
      expect(result).toHaveLength(1);
    });
    ...
    it("should ignore images without any src attribute", () => {
      const html =
        '<img alt="no source"><img src="https://example.com/valid.jpg">';
      const result = extractImagesFromHtml(html);
      expect(result).toHaveLength(1);
